...
function switchItemColor(e) {
  ...
  const closestItemImage = e.target
    .closest('.item-card')
    .querySelector('.item-img');
  const currentItemColor = closestItemImage.src
    .match(ITEM_COLOR_REGEX)[0]
    .slice(1, -1);

  if (radioValue === currentItemColor) return;

  let newImageSource = closestItemImage
    .getAttribute('src')
    .replaceAll(ITEM_COLOR_REGEX, srcColorReplacement);
